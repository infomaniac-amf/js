...
//  var properties = ObjectUtil.isSerializable(data) ? data.export() : ObjectUtil.getObjectKeys(data);
  var properties = ObjectUtil.getObjectKeys(data);

  // write object info & class name
  this.serializeInt(11);
  this.serializeString(ObjectUtil.getClassName(data, this.options), false);
...
Serializer.prototype.serializeByteArray = function(data) {
  if(!('getData' in data)) {
    throw new Error('Invalid ByteArray data provided');
  }
