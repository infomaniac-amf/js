...
Serializer.prototype.serialize = function(data, includeType, forceType) {
  ...
  switch(type) {
    ...
    default:
      throw new exception.SerializationException('Unrecognized AMF type [' + type + ']');
  }
...
Serializer.prototype.serializeInt = function(data) {
  if(data < Spec.MIN_INT || data > Spec.MAX_INT) {
    throw new exception.SerializationException('Integer out of range: ' + data);
  }
...
Serializer.prototype.serializeByteArray = function(data) {
  if(!('getData' in data)) {
    throw new exception.SerializationException('Invalid ByteArray data provided');
  }
